...
ID3v2_tag* load_tag(const char* file_name)
{
    ...
    buffer = (char*) safeAlloc((10+header_size) * sizeof(char));
    if(buffer == NULL) {
...
ID3v2_tag* load_tag_with_buffer(char *bytes, int length)
{
    ...
    tag->raw = (char*) safeAlloc(tag->tag_header->tag_size * sizeof(char));
 ...
 * Setter functions
 */
void set_text_frame(char* data, char encoding, const char* frame_id, ID3v2_frame* frame)
{
    char *frame_data;
    ...
    frame_data = (char*) safeAlloc(frame->size * sizeof(char));
    frame->data = (char*) safeAlloc(frame->size * sizeof(char));

    sprintf(frame_data, "%c%s", encoding, data);
    memcpy(frame->data, frame_data, frame->size);

    safeFree((void**)&frame_data);
}
...
void set_album_cover_frame(char* album_cover_bytes, const char* mimetype, int picture_size, ID3v2_frame* frame)
{
    char *frame_data;
    int offset;
...
void tag_set_album_cover(const char* filename, ID3v2_tag* tag)
{
    ...
    album_cover_bytes = (char*) safeAlloc(image_size * sizeof(char));
    ...
    safeFree((void**)&album_cover_bytes);
}

void tag_set_album_cover_from_bytes(char* album_cover_bytes, const char* mimetype, int picture_size, ID3v2_tag* tag)
{
