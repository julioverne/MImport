...
{
	// no needed, NSMutableData will autorelease
...
{
	// alloc with help of NSMutableData and retrive pointer of memory, malloc() seems to cause random app crash
...
char* itob(int integer)
{
    int i;
    int size = 4;
    char* result = (char*) safeAlloc(sizeof(char) * size);
...
void free_tag(ID3v2_tag* tag)
{
    ...
    safeFree((void**)&tag->raw);
    safeFree((void**)&tag->tag_header);
    list = tag->frames;
    while(list != NULL)
    {
        if (list->frame) safeFree((void**)&list->frame->data);
        safeFree((void**)&list->frame);
        list = list->next;
    }
    safeFree((void**)&list);
    safeFree((void**)&tag);
...
void genre_num_string(char* dest, char *genre_data)
{
    ...
    const char *genre = convert_genre_number(genre_id);
    strcpy(dest, genre);
}
...
{
    uint16_t* result = (uint16_t*) safeAlloc(size * sizeof(uint16_t));
...
char* get_path_to_file(const char* file)
{
    ...
    char* file_path = (char*) safeAlloc(size * sizeof(char));
